...
def CorrFunc(Njob=Njob,k0=1, order=0,name_corr = filename):
    ...
    pwly = 1.2*0.01*np.power(pwlx,-0.7)
    #plt.plot(pwlx,pwly)
    
    
    #compare with Johan's result:
    #johan_random = np.loadtxt('/global/homes/h/huikong/obiwan_test/obiwan_corr/corrfunc_data/Johan_random.txt').transpose()
    #plt.errorbar(johan_random[0],johan_random[1],johan_random[2])
    #plt.plot(johan_random[0],johan_random[2])
    #plt.plot(johan_random[0],johan_random[3])
    
    plt.gca().set_xlim((0.009, 5))
